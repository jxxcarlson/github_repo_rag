...
var $author$project$ElmParser$encodeChunk = function (chunk) {
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'type',
				$elm$json$Json$Encode$string(chunk.type_)),
				_Utils_Tuple2(
				'name',
				$elm$json$Json$Encode$string(chunk.name)),
				_Utils_Tuple2(
				'code',
				$elm$json$Json$Encode$string(chunk.code)),
				_Utils_Tuple2(
				'startLine',
				$elm$json$Json$Encode$int(chunk.startLine)),
				_Utils_Tuple2(
				'endLine',
				$elm$json$Json$Encode$int(chunk.endLine)),
				_Utils_Tuple2(
				'calls',
				A2($elm$json$Json$Encode$list, $elm$json$Json$Encode$string, chunk.calls)),
				_Utils_Tuple2(
				'imports',
				A2($elm$json$Json$Encode$list, $elm$json$Json$Encode$string, chunk.imports))
			]));
};
var $author$project$ElmParser$encodeResult = function (result) {
	if (result.$ === 'Ok') {
		var chunks = result.a;
		return $elm$json$Json$Encode$object(
			_List_fromArray(
				[
					...
					_Utils_Tuple2(
					'value',
					A2($elm$json$Json$Encode$list, $author$project$ElmParser$encodeChunk, chunks))
				]));
	} else {
		var error = result.a;
		return $elm$json$Json$Encode$object(
...
var $author$project$ElmParser$Chunk = F7(
	function (type_, name, code, startLine, endLine, calls, imports) {
		return {calls: calls, code: code, endLine: endLine, imports: imports, name: name, startLine: startLine, type_: type_};
	});
...
var $author$project$ElmParser$extractCode = F2(
	function (source, range) {
		...
		return A2(
			...
			A2(
				$elm$core$List$drop,
				startLine,
				A2($elm$core$List$take, endLine, lines)));
	});
...
var $author$project$ElmParser$extractChunks = F2(
	function (source, file) {
		var imports = A2(
			...
			A2(
				$elm$core$List$map,
				function (imp) {
					var importValue = $stil4m$elm_syntax$Elm$Syntax$Node$value(imp);
					var moduleNameNode = importValue.moduleName;
					var moduleNameList = $stil4m$elm_syntax$Elm$Syntax$Node$value(moduleNameNode);
					return A2($elm$core$String$join, '.', moduleNameList);
				},
				file.imports));
		return A2(
			$elm$core$List$concatMap,
			function (decl) {
				var _v0 = $stil4m$elm_syntax$Elm$Syntax$Node$value(decl);
				switch (_v0.$) {
					case 'FunctionDeclaration':
						var _function = _v0.a;
						var range = $stil4m$elm_syntax$Elm$Syntax$Node$range(decl);
						var implementation = $stil4m$elm_syntax$Elm$Syntax$Node$value(_function.declaration);
						var name = $stil4m$elm_syntax$Elm$Syntax$Node$value(implementation.name);
						return _List_fromArray(
							[
								A7(
								$author$project$ElmParser$Chunk,
								'function',
								name,
								A2($author$project$ElmParser$extractCode, source, range),
								range.start.row,
								range.end.row,
								_List_Nil,
								imports)
					...
					case 'AliasDeclaration':
						var typeAlias = _v0.a;
						var range = $stil4m$elm_syntax$Elm$Syntax$Node$range(decl);
						var name = $stil4m$elm_syntax$Elm$Syntax$Node$value(typeAlias.name);
						return _List_fromArray(
							[
								A7(
								$author$project$ElmParser$Chunk,
					...
					case 'CustomTypeDeclaration':
						var typeDecl = _v0.a;
						var range = $stil4m$elm_syntax$Elm$Syntax$Node$range(decl);
						var name = $stil4m$elm_syntax$Elm$Syntax$Node$value(typeDecl.name);
						return _List_fromArray(
							[
								A7(
								$author$project$ElmParser$Chunk,
...
var $author$project$ElmParser$parseElmFile = function (source) {
	var _v0 = $stil4m$elm_syntax$Elm$Parser$parse(source);
	if (_v0.$ === 'Ok') {
		var rawFile = _v0.a;
		var file = A2($stil4m$elm_syntax$Elm$Processing$process, $stil4m$elm_syntax$Elm$Processing$init, rawFile);
		return $elm$core$Result$Ok(
			A2($author$project$ElmParser$extractChunks, source, file));
	} else {
		return $elm$core$Result$Err('Failed to parse Elm file');
	}
};
var $elm$json$Json$Decode$string = _Json_decodeString;
var $author$project$ElmParser$parseFile = _Platform_incomingPort('parseFile', $elm$json$Json$Decode$string);
var $author$project$ElmParser$parseResult = _Platform_outgoingPort('parseResult', $elm$core$Basics$identity);
var $elm$json$Json$Decode$succeed = _Json_succeed;
var $elm$core$Platform$worker = _Platform_worker;
var $author$project$ElmParser$main = $elm$core$Platform$worker(
	{
		init: function (_v0) {
			return _Utils_Tuple2(_Utils_Tuple0, $elm$core$Platform$Cmd$none);
		},
		subscriptions: function (_v1) {
			return $author$project$ElmParser$parseFile($author$project$ElmParser$parseElmFile);
		},
		update: F2(
			function (msg, _v2) {
				return _Utils_Tuple2(
					_Utils_Tuple0,
					$author$project$ElmParser$parseResult(
						$author$project$ElmParser$encodeResult(msg)));
			})
	});
_Platform_export({'ElmParser':{'init':$author$project$ElmParser$main(
